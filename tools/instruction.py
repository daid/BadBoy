...
class Instruction:
    def __init__(self, rom, address):
        ...
        if op == 0x02: self.__set(LD, target=Ref(BC), source=A)
        if op == 0x12: self.__set(LD, target=Ref(DE), source=A)
        if op == 0x22: self.__set(LD, target=Ref("HL+"), source=A)
        if op == 0x32: self.__set(LD, target=Ref("HL-"), source=A)
        ...
        if op == 0x36: self.__set(LD, target=Ref(HL), value=self.__getUInt8())
        ...
        if op == 0x08: self.__set(LD, target=Ref(self.__getUInt16()), source=SP)
        ...
        if op == 0x0A: self.__set(LD, target=A, source=Ref(BC))
        if op == 0x1A: self.__set(LD, target=A, source=Ref(DE))
        if op == 0x2A: self.__set(LD, target=A, source=Ref("HL+"))
        if op == 0x3A: self.__set(LD, target=A, source=Ref("HL-"))
        ...
        if op == 0x70: self.__set(LD, target=Ref(HL), source=B)
        ...
        if op == 0x71: self.__set(LD, target=Ref(HL), source=C)
        ...
        if op == 0x72: self.__set(LD, target=Ref(HL), source=D)
        ...
        if op == 0x73: self.__set(LD, target=Ref(HL), source=E)
        ...
        if op == 0x74: self.__set(LD, target=Ref(HL), source=H)
        ...
        if op == 0x75: self.__set(LD, target=Ref(HL), source=L)

        if op == 0x46: self.__set(LD, target=B, source=Ref(HL))
        if op == 0x56: self.__set(LD, target=D, source=Ref(HL))
        if op == 0x66: self.__set(LD, target=H, source=Ref(HL))
        ...
        if op == 0x77: self.__set(LD, target=Ref(HL), source=A)
        ...
        if op == 0x4E: self.__set(LD, target=C, source=Ref(HL))
        if op == 0x5E: self.__set(LD, target=E, source=Ref(HL))
        if op == 0x6E: self.__set(LD, target=L, source=Ref(HL))
        if op == 0x7E: self.__set(LD, target=A, source=Ref(HL))
        ...
        if op == 0x86: self.__set(ADD, target=A, source=Ref(HL))
        ...
        if op == 0x8E: self.__set(ADC, target=A, source=Ref(HL))
        ...
        if op == 0x96: self.__set(SUB, target=A, source=Ref(HL))
        ...
        if op == 0x9E: self.__set(SBC, target=A, source=Ref(HL))
        ...
        if op == 0xA6: self.__set(AND, target=A, source=Ref(HL))
        ...
        if op == 0xAE: self.__set(XOR, target=A, source=Ref(HL))
        ...
        if op == 0xB6: self.__set(OR, target=A, source=Ref(HL))
        ...
        if op == 0xBE: self.__set(CP, target=A, source=Ref(HL))
        ...
        if op == 0xE2: self.__set(LDH, target=Ref(C), source=A)
        if op == 0xF2: self.__set(LDH, target=A, source=Ref(C))
        ...
        if op == 0xEA: self.__set(LD, target=Ref(self.__getUInt16()), source=A)
        if op == 0xFA: self.__set(LD, target=A, source=Ref(self.__getUInt16()))
    ...
    def __decodeCB(self, op):
        ...
        if (op & 0x07) == 6: self.target = Ref(HL)
    ...
    def format(self, symbol_table):
        ...
        if self.type in (LDH):
            if self.target == A:
                return self.__format(self.target, "[$%04x]" % (self.source | 0xFF00))
            return self.__format("[$%04x]" % (self.target | 0xFF00), self.source)
